...
if (isset($_POST['log_in'])) {
  ...
  if(empty($username)){
   ...
   if(empty($username)||empty($user_password)){
    ...
    $sql_username = "SELECT * FROM child_data WHERE email ='$username'";
    ...
    if( $data -> num_rows == 1) {
        ...
        if($user_password == $f_data['password']){
            ...
            $_SESSION['child_name'] = $f_data['child_name'];
            $_SESSION['father_name'] = $f_data['father_name'];
            $_SESSION['mother_name'] = $f_data['mother_name'];
  ...
  <div class="container ">
    <div style="width: 420px;" class="card mx-auto mt-5">
      ...
      <div class="form">
        ...
        <label id="forgotpwd">Worker Login<a href="worker-login.php"> Click Here</a></label>
        <label id="forgotpwd" class="text-right float-right">Registration<a href="child-registration.php"> Click Here</a></label>
      </form>
    </div>
  </div>
      </div>
